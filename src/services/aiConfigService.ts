...
import {
  ...
  GetInteractionLogsResponse,
  GetModelErrorsResponse,
  getModelsHealthResponse,
  UpdateAIModelDto
} from '~/shared/types/services/ai-config-service.type';
...
export const getModelErrors = async (limit: number = 20): Promise<GetModelErrorsResponse> => {
  const response = await axios.get(`${PREFIX}/errors?limit=${limit}`);
  return response.data;
};

export const getInteractionLogs = async (
  limit: number = 50
): Promise<GetInteractionLogsResponse> => {
 ...
 */
export const createModel = async (modelData: CreateAIModelDto): Promise<AIModel> => {
 ...
 */
export const updateModel = async (modelId: number, updates: UpdateAIModelDto): Promise<AIModel> => {
