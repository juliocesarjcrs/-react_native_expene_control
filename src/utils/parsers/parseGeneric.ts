...
function fallbackHeuristicProcessing(joined: string): Product[] {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const descRegex =
      /(?:\d+\s+)?(?:Und\.?\s+)?([A-ZÁÉÍÓÚÑa-záéíóúñ().,/\- ]{3,}?)(?=\s+(?:Und\.?|$))/g;
    let descMatch;

    while ((descMatch = descRegex.exec(fragment)) !== null) {
      const description = formatDescription(descMatch[1].replace(/\s{2,}/g, ' ').trim());

      // Filtra encabezados y descripciones vacías o muy cortas
      if (
        description.length > 2 &&
        !/^(precio|cantidad|producto|un|und|total|item|subtotal|descuento|ahorro|valor|pago|otro|forma de pago|pagado|cambio)/i.test(
          description
        ) &&
